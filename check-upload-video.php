...
if (!isset($_POST['select-branch'])){ header("Location:  officer-upload-videos.php?error=Please Select Branch"); exit(); }
   ...
   if ($branch == "Select Branch"){ header("Location:  officer-upload-videos.php?error=Please Select Branch"); exit(); }
       ...
       if(in_array($extension,$extensions_arr) ){
          ...
          if(($_FILES['file']['size'] >= $maxsize) || ($_FILES["file"]["size"] == 0)) {
             ...
             if(move_uploaded_file($_FILES['file']['tmp_name'],$target_file)){
               $sql = "INSERT INTO videos(date, time, serviceID, location, description) 
               VALUES('".$date."','".$time."','".$serviceID."','".$target_file."','".$description."')";
